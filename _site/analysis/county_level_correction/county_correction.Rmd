...
knitr::opts_chunk$set(echo = TRUE, message = FALSE, warning = FALSE, eval=FALSE, fig.width = 11)
...
```{r constrain priors for every biweek,  eval= TRUE}
...
```{r compare melded distributions to original}

# plot melded distributions and compare to original
original <- map_df(6:30, ~{ 
  tibble(biweek = .x,
         beta =  sample_beta_density(
        1e4,
        mean = prior_params$beta_mean,
        sd = prior_params$beta_sd),
        source = "original prior")})
 
 
do.call(bind_rows, priors_constrained_by_week) %>%
