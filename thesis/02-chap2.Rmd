...
```{r,fig.width=7, fig.align='center', fig.cap = "\\label{fig:theta}", fig.pos = "H"}
...
```{r create melded figure, eval=FALSE}
...
```{r, fig.cap = "\\label{fig:melded}", fig.width=7}

knitr::include_graphics(here::here("thesis/figure/melded.png"))
...
title_gg <- ggplot() + 
  labs(title =latex2exp::TeX("Distribution of Resample $Z_1,..., Z_r$ when Increasing the Sample Size $m$")) + 
...
```{r, fig.cap ="\\label{fig:prepostsamp}", fig.width=7}
...
```{r,fig.cap = "\\label{fig:trunc}", fig.show='hold', out.width='100%'}
...
```{r, fig.cap="\\label{fig:loess}LOESS curve fitted with  a span of 0.2. ", echo = FALSE, fig.asp = .6, cache = TRUE}
...
```{r, fig.cap="\\label{fig:smooth-spans}", fig.asp = .67, cache = TRUE }
...
```{r, warning = FALSE, fig.cap = "\\label{fig:weights} The only values with nonzero weights are those within the interval $(500 - \\alpha (n), 500 - \\alpha (n))$. That is, the proportion $\\alpha$ of the data points closest to $x^*$ will have nonzero weights.", fig.width =6, fig.asp = .6, cache = TRUE}
...
```{r, fig.cap = "\\label{fig:ex-poly}", fig.asp = .52, fig.width = 5, cache = TRUE}
...
```{r, fig.cap = "\\label{smooth-functions}", cache = TRUE}
...
```{r loess-all, fig.cap = "\\label{loess-all}", fig.asp = .6, cache = TRUE}
...
```{r create fig original, eval=FALSE}
...
```{r, fig.width=7, fig.cap = "\\label{fig:original}"}
...
```{r create fig:compare-beta-params, eval=FALSE}
...
title_gg <- ggplot() + 
  labs(title =latex2exp::TeX("Comparing Approaches for Estimating $f_X(x)$ for Different Beta Distributions", 
                             bold = TRUE)) + 
...
```{r, fig.cap = "\\label{fig:compare-beta-params}", out.width="90%" }
...
```{r create depends_on_x.jpeg,eval=FALSE}

jpeg(here::here('thesis/figure/depends_on_x.jpeg'), width = 800, height =500, quality=400)
df %>%
...
```{r, fig.cap = "\\label{fig:depends-on-x}", fig.width=7}


knitr::include_graphics(here::here('thesis/figure/depends_on_x.jpeg'))
