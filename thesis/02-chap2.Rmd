...
tibble(x = seq(0, 1, by = 10^(-4)),
  ...
  ggplot(aes(x=x, y = y)) +
  geom_area(alpha = .8) + 
  theme_c(plot.title =element_text(size = 14)) +
  labs(x = "Value", y = "Probability Density", 
...
Applying the definition of conditional probability to  the term \\ $P(test_+|S_0, \text{untested})P(S_0|\text{untested})$ in the numerator,
...
With same reasoning, we can simplify \\$P(test_+|S_1, \text{untested})P(S_1|\text{untested}) = P(S_1, test_+|\text{untested})$, giving us
...
```{r, warning = FALSE, fig.cap = "\\label{fig:weights} We see that the only values with nonzero weights are those within the interval $(500 - \\alpha (n), 500 - \\alpha (n))$, that is, the proportion $\\alpha$ of the data points closest to $x^*$.", fig.asp= 0.6}
...
data %>%
  ...
  geom_vline(aes(xintercept = lower),
                 color = "darkred", alpha = .6) +
  annotate(x = 330, geom= "text", y = .5, 
           label = TeX("$500 - (\\alpha)(n)$"), parse =TRUE) +
  annotate(x = 670, geom= "text", y = .5, 
           label = TeX("$500 + (\\alpha)(n)$"), parse =TRUE) +
  theme_bw()+
  theme(plot.title = element_text(hjust = .5, face="bold", size = 14),
        plot.subtitle = element_text(hjust = .5, face="italic", size = 14),
...
```{r, fig.cap = "\\label{fig:ex-poly}", fig.asp = .6}
