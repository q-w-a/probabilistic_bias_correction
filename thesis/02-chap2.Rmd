...
### Prior and Induced Prior Distributions for $P(S_0|\text{test}_+,\text{untested})$ 
...
```{r, fig.cap = "\\label{fig:melded}", fig.height = 9, fig.width = 6.5}
...
```{r, warning = FALSE, fig.cap = "\\label{fig:weights} We see that the only values with nonzero weights are those within the interval $(500 - \\alpha (n), 500 - \\alpha (n))$. That is, the proportion $\\alpha$ of the data points closest to $x^*$ will have nonzero weights.", fig.asp= 0.65, fig.width =7.5}
