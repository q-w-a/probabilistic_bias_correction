...
# 
# pal1 <-viridis_pal(option="viridis", end = .9, begin=.5)(12)
# pal2 <- viridis_pal(option="rocket", end = .9, begin=.3)(15)
# pal3 <- viridis_pal(option="magma", end = .9, begin=.3)(15)
# pal4 <- viridis_pal(option="mako", end = .8, begin=.3)(15)
# 
# 
# rankpal <- c(pal1, pal2, pal3,pal4)
...
#              color = state)) +
#   geom_line(size = .7, show.legend=FALSE, alpha = .7) +
#   theme_bw() +
...
# 
#  ggsave(width=8, height = 6,"./thesis/figure/testing_all.jpeg", dpi=600)
# 
...
testing  %>%
   mutate(`Number of Tests / Population Size` = total/population) %>%
  group_by(state) %>%
  mutate(m = median(`Number of Tests / Population Size`)) %>%
  ungroup() %>%
  mutate(max=max(m,na.rm=TRUE),
         min = min(m,na.rm=TRUE)) %>%
  rename(code=state) %>%
  left_join(codes) %>%
  mutate(state_name = ifelse(m == max | m==min, state, NA)) %>%
  ggplot(aes(x=date,  y=`Number of Tests / Population Size`,
             group=state)) +
  geom_line(alpha = .2) +
  geom_line(aes(color=state_name),
            linewidth=1.3) +
  scale_color_manual(na.value=NA, values=c("#900C3F", "#5098C2"),
                     breaks = c("Mississippi", "Rhode Island"),
                     labels= c("Mississippi", "Rhode Island"),
                     name='')+
  theme_bw() +
  theme(axis.title = element_text(size = 22),
          legend.position = "top",
...
ggsave(width=10, height = 6,"./thesis/figure/testing_all.jpeg", dpi=600)
