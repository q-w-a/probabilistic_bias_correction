...
crosscorr <- function(lag_val, x, y) {
  n <- length(x)
  sq_diff_x <- (sum( (x -mean(x))  %*% (x -mean(x)) ))
  sq_diff_y <- (sum( (y -mean(y))  %*% (y -mean(y)))) 
  denom <- sqrt(sq_diff_x*sq_diff_y)                                     
  num <- (x[1:(n-lag_val)] - mean(x)) %*% (y[(lag_val+1):n] - mean(y))
  as.numeric(num /denom)
}
...
n <- length(x)
var_x <- (sum( (x -mean(x))  %*% (x -mean(x)) )) /(n-1)
var_y <- (sum( (y -mean(y))  %*% (y -mean(y))))  / (n-1)
denom <- sqrt(var_x*var_y)                                     
num <- (x[1:(n-lag_val)] - mean(x)) %*% (y[(lag_val+1):n] - mean(y)) / (n-1)
as.numeric(num /denom)
