...
fips_include<- wjoined %>%
  filter(grepl("Suffolk|Berkshire|Worcester",name)) %>%
  pull(fips) %>%
  unique()

plt1 <-  compare_county_wastewater_covidestim(county_fips =fips_include[1], w_data =wjoined, option = "ribbon")
plt2 <- compare_county_wastewater_covidestim(county_fips =fips_include[2], w_data =wjoined, option = "ribbon")

plt2


ggsave(here('presentation/figure/suffolk_wastewater.jpeg'), width=15, height=8)


plt3 <- compare_county_wastewater_covidestim(county_fips =fips_include[3], w_data =wjoined, option = "ribbon")
...
cols <- c("red",pal)
names(cols ) <- c("$observed$", unique(as.character(lags_all_pb$vlabel)))
...
lags_all_pb %>%
  ...
  scale_fill_manual(values=cols,
                    ...
                    labels=TeX(names(cols))) +
  theme_bw() +
  theme_c() +
  theme(plot.title = element_text(size=19,hjust=.5, face="plain"),
...
ggsave(here("thesis/figure/correlation_observed_pb.jpeg"), height = 8, width=13.2)
...
name_order <- lags_all_pb %>%
  bind_rows(lags_all_observed) %>%
   mutate(source = ifelse(!is.na(vlabel), vlabel, comparison)) %>%
  group_by(county_name) %>%
  mutate(m = median(max_corr)) %>%
  ungroup() %>%
  select(m,county_name) %>%
  arrange(desc(m)) %>%
  distinct() %>%
  pull(county_name)
...
lags_all %>% 
  ...
  theme(plot.title = element_text(size=19,hjust=.5, face="plain"),
    # axis.text.x = element_text(angle  = 60, vjust = .4),
    axis.text.x = element_text(angle  = 20, vjust = .4, size=16),
    axis.title.x = element_text(margin = margin(8,3,3,3), size=18),
...
ggsave(here("thesis/figure/correlation_observed_pb_covidestim.jpeg"), width=13, height=8)
