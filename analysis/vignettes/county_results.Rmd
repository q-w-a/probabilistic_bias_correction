...
knitr::opts_chunk$set(echo = TRUE, fig.height=10, fig.width =14, message=FALSE, warning=FALSE, cache=TRUE)
...
### Ratio of Estimated to Observed Over Time


```{r}

pal <- c( mako(6, begin=.1,end=.9), magma(7, begin=.1, end=.9))
pal <- pal[sample(1:length(pal))]
...
ranks %>%
  ...
  geom_point() +
  geom_line() +
  theme_c(legend.title = element_text(size=16, face="bold")) +
...
## Compare Trends in the Ratio of Estimated to Observed Infections to Trends in Infections


```{r, fig.height= 23, fig.width = 14}
...
title_gg <- ggplot() + 
  ...
  theme(plot.title=element_text(face="bold", hjust = .5, size = 25, margin =margin(5,0,2,0)))


title_row <- plot_grid(title_gg,legend, nrow=2,rel_heights=c(.6,.4))
grid <- plot_grid(plotlist = pltlist, ncol=2)
...
```{r pb-intervals-ma, fig.width=17, include=FALSE}
...
joined %>%
 ...
 filter(in_interval) %>%
  ...
  theme_c(axis.text.x = element_text( size =16, angle =0),
          ...
          legend.position ="top",
          plot.title = element_text(size = 22, face="bold", margin = margin(2,0,3,0))) +
...
### Heatmap for Ratio of Estimated Infections to Observed
...
### Compare Trends in Ratio of Estimated Infections to Observed to Trends in Infections
...
plot_grid(title_row,grid, rel_heights = c(.08,.91),nrow=2)

ggsave(here('thesis/figure/compare-ratio-to-total-mi.pdf'), height=9,width=11)
...
subset %>%
  ...
  geom_point(aes(y=m, color='Median'),size=2) +
  facet_wrap(~pop) +
  theme_c() +
...
ggsave(here('thesis/figure/mi-ratio-pop.jpeg'), width=11,height=10,dpi=600)
...
mi_concordance_by_county  %>%
  ...
  labs(y = "", x="Proportion Containing Covidestim Median",
       fill="",
       title = "Proportion Containing Covidestim Median for the Most Concordant Version",
       subtitle="By County") 
