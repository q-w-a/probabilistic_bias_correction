...
covidestim <- read_csv('https://media.githubusercontent.com/media/covidestim/covidestim-sources/master/data-sources/jhu-states.csv') %>%
...
tests_weekly <- readRDS("data/state_level/tests_daily_all_states.RDS") %>%
  mutate(week=week(date),
         year=year(date)) %>%
  mutate(week = ifelse(week==53 & year == 2021,52,week)) %>%
  group_by(year,week, state) %>%
...
cowplot::plot_grid(plotlist=pltlist, ncol=5)
