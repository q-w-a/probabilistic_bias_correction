...
variant <- "https://data.cdc.gov/resource/jr58-6ysp.json?$limit=50000&$where=week_ending between '2021-02-18T00:00:00.000' and '2022-02-18T00:00:00.000'&usa_or_hhsregion=USA"
...
variant <- readRDS('data/variant_prop.RDS')
# 
# test %>%
#   flatten()
# 
...
variant <- variant %>% 
  ...
  group_by(variant, week_end_date, variant_category) %>% 
  slice_max(n=1, order_by=creation_date) %>%
  group_by(variant_category, week_end_date) %>%
...
variant %>%
  ...
  ggplot(aes(x=week_end_date, y = share,
