...
fb_symptoms <- readRDS("data/state_level/screeningpos_all_states.RDS")
...
symp %>% 
  ...
  geom_line() +
  theme_c(axis.text.x = element_text(angle =30),
          axis.title = element_text(size =16)) +
  facet_wrap(~toupper(state)) +
  labs(title = "Empirical Estimate of Beta\n from COVID-19 Trends and Impact Survey Data",
       y = "Screening Test Positivity / Overall Test Positivity")

ggsave('thesis/figure/ctis_beta_states.pdf', height = 10, width = 10)
...
symp_imputed %>%
  ...
  ggplot(aes(x=date, y = imputed, color = was_na)) +
