...
fb_sample_size <- fb_sample_size %>% 
  group_by(state) %>%
  mutate(mean_sampled = mean(prop_sampled)) %>%
  ungroup() %>%
...
fb_sample_size %>%
  mutate(quantile_sampled = as_factor(
    quantile_sampled)) %>%
  ggplot(aes(x = prop_sampled,
             fill =quantile_sampled)) +
  geom_density() +
  facet_wrap(~fct_reorder(state, as.numeric(quantile_sampled)))
  
  
  
  group_by(state) %>%
  mutate(mean_sampled = mean(value))
