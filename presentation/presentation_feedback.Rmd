...
Scenario 1: Suppose:
* $P(\text{tested})$ is constant 
* $P(S_1|\text{tested})$ is increasing 
* $P(S_1)$ is decreasing
...
sim_start %>%
  ...
  labs(x = TeX("$\\alpha$"), 
       y= TeX("$\\beta$"),
       color = TeX("$Pr(test_+|tested)$"),
       title = TeX("$Pr(S_1)$, $Pr(test_+|tested)$, $Pr(test_+|untested,S_0)$ All Increasing")) +
  viridis::scale_color_viridis(option="mako", direction=-1, end=.95) 


ggsave(here('presentation/figure/alpha_beta_negative_corr.jpeg'),width=10,height=5.5)



sim_start <- tibble(tested = rep(TESTED, reps),
       untested = 1-tested, 
       pos = seq(0.02,0.04, length=reps),
       s= seq(.05,.02, length=reps),
       s_given_tested =.7,
       # pos_given_a_untested = seq(.003,.01, length=30),
        pos_given_a_untested = seq(.003,.01, length=reps),
       pos_given_tested =seq(0.05,0.08, length =reps))
...
sim_start %>%
  ...
  labs(x = TeX("$\\alpha$"), 
       y= TeX("$\\beta$"),
       color = TeX("$Pr(test_+|tested)$"),
       title = TeX("$Pr(S_1)$, $Pr(test_+|tested)$ Increasing, $Pr(test_+|untested,S_0)$ Decreasing")) +
  viridis::scale_color_viridis(option="mako", direction=-1, end=.95) 


ggsave(here('thesis/figure/alpha_beta_negative_corr.jpeg'),width=10,height=5.5)
...
sim_start %>%
  ...
  labs(x = TeX("$\\alpha$"), 
       y= TeX("$\\beta$"),
       color = TeX("$Pr(test_+|tested)$"),
       title = TeX("$Pr(S_1)$ and $Pr(test_+|tested)$ are Increasing, but $Pr(test_+|untested,S_0)$ is Decreasing")) +
  viridis::scale_color_viridis(option="mako", direction=-1, end=.95) 



ggsave(here('presentation/figure/alpha_beta_positive_corr.jpeg'),width=10,height=5.5)
